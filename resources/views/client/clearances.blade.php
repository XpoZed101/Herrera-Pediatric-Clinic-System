<x-layouts.app :title="__('Clearances')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="rounded-2xl bg-gradient-to-br from-indigo-50 to-white dark:from-zinc-800 dark:to-zinc-900 border border-neutral-200 dark:border-neutral-700 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h2 class="text-xl font-semibold tracking-tight">{{ __('Medical Clearances') }}</h2>
                    <p class="text-neutral-600 dark:text-neutral-300 text-sm">{{ __('Download a clearance document for each visit or record.') }}</p>
                </div>
                ...
                <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 overflow-hidden">
                    <div class="px-4 py-4 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="grid place-items-center h-10 w-10 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 text-sm font-semibold">
                                {{ $initials ?: '👶' }}
                            </div>
                    ...
                    <div class="px-4 py-4">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-sm font-semibold tracking-tight">✅ {{ __('Clearances by visit') }}</h3>
                        ...
                        @if(isset($medicalRecords) && $medicalRecords->count())
                            <div class="max-h-[60vh] overflow-auto rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm">
                                <table class="min-w-full text-sm">
                                    <thead class="sticky top-0 bg-gradient-to-r from-indigo-50 to-sky-50 dark:from-zinc-800 dark:to-zinc-700 backdrop-blur supports-[backdrop-filter]:bg-indigo-50/70 dark:supports-[backdrop-filter]:bg-zinc-800/50">
                                        ...
                                        @foreach($medicalRecords as $record)
                                            <tr class="odd:bg-white even:bg-neutral-50 dark:odd:bg-zinc-900 dark:even:bg-zinc-800/50 hover:bg-neutral-100/60 dark:hover:bg-zinc-700/60 transition-colors">
                                                ...
                                                <td class="px-3 py-2 text-neutral-800 dark:text-neutral-200 text-xs">
                                                    <a href="{{ route('client.medical-records.clearance.pdf', $record) }}" target="_blank" class="inline-flex items-center gap-1 rounded-md bg-emerald-600 text-white px-2 py-1 hover:bg-emerald-700" title="{{ __('Download Clearance PDF') }}">
                                                        <flux:icon.shield-check variant="mini" /> {{ __('Clearance PDF') }}
                                                    </a>
                                                </td>
                                            </tr>
                                        @endforeach
                ...
                <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 overflow-hidden">
                    <div class="px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
                        <div class="flex items-center justify-between">
                            <h3 class="text-sm font-semibold tracking-tight">{{ __('Clearances') }}</h3>
                        </div>
                    </div>
                    <div class="px-4 py-3">
